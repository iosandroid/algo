...
def ZigZag0(price, minsize, plotting = True):
    ...
    ZigZagInternal0(X, pref, 0, len(pref), 0)

    X.sort(key = lambda tup: tup['tmin'])

    Xs = []
    for i in range(len(X)-1):
        if (math.fabs(X[i]['slice']) > minsize) and (X[i]['delta'] > MinTimeDelta):
...
def ZigZagInternal0(Z, pref, pmin, pmax, depth):
    ...
    for i in range(count):
        for p in range(i, count-1):
            ...
            if Z0 is None:
                Z0 = { 'tmin' : pref0[i]['i'], 'tmax' : pref0[p + 1]['i'], 'delta' : delta, 'slice' : slice, 'label' : 1 if slice > 0 else -1 }                

            elif math.fabs(slice) > math.fabs(Z0['slice']) and (delta < MaxTimeDelta):
                Z0 = { 'tmin' : pref0[i]['i'], 'tmax' : pref0[p + 1]['i'], 'delta' : delta, 'slice' : slice, 'label' : 1 if slice > 0 else -1 }
    ...
    if Z0 is not None:    
        ...
        if zmin - pmin > 1:            
            ZigZagInternal0(Z, pref, pmin, zmin, depth + 1)

        if pmax - zmax > 1:            
            ZigZagInternal0(Z, pref, zmax, pmax, depth + 1)
