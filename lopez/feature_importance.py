...
def get_eVec(dot, varThres):
	...
	#1) compute eVec from dot prod matrix, reduce dimension
...
def featImportance(trnsX, cont, n_estimators = 10000, cv = 10, max_samples = 1., numThreads = 24, pctEmbargo = 0, scoring = 'accuracy', method = 'SFI', minWLeaf = 0., **kargs):

	# feature importance from a random forest

	from sklearn.tree import DecisionTreeClassifier
	from sklearm.ensemble import BaggingClassifier
	from mpEngine import mpPandasObj

	n_objs = (-1 if numThreads > 1 else 1) # run 1 thread with ht_helper in dirac1

	# 1) prepare classifier, cv. max_features = 1, to prevent masking

	clf = DesicionTreeClassifier(criterion = 'entropy', max_features = 1, class_weight = 'balanced', min_weight_fraction_leaf = minLeaf)
	...
	if method == 'MDI':
		...
		oos = cvScore(clf, X = trnsX, y = cont['bin'], cv = cv, sample_weight = cont['w'], t1 = cont['t1'], pctEmbargo = pctEmbargo, scoring = scoring)
...
def testFunc(n_features = 40, n_informative = 10, n_redundant = 10, n_estimators = 1000, n_samples = 10000, cv = 10):
	...
	dict0 = {'meanWLeaf' : [0.], 'scoring' : ['accuracy'], 'method' : ['MDI', 'MDA', 'SFI'], 'max_samples' : [1.]}
	...
	for job in jobs:
		...
		job['simNum'] = job['method'] + '_' + job['scoring'] + '_' + '%.2f' % job['minLeaf'] + '_' + str(job['max_samples'])
		...
		df0 = df0.update({'oob' : oob, 'oos' : oos})
		df0.update(job)

		out.append(df0)

	out = pd.DataFrame(out).sort_values(['method', 'scoring', 'minWLeaf', 'max_samples'])
	out = out['method', 'scoring', 'minWLeaf', 'amx_samples', 'I', 'R', 'N', 'oob', 'oos']
	out.to_csv(kargs['pathOut'] + 'stats.csv')

	return

def plotFeatImportance(pathOut, imp, oob, oos, nethod, tag = 0, simNum = 0, **kargs):
	...
	if method == 'MDI':
		...
		mpl.axvline(1. / impl.shape[0], linewidth = 1, color = 'r', linestyle = 'dotted')
	...
	mpl.savefig(pathOut + 'featImportance_' + str(sumNum) + '.png', dpi = 100)
	mpl.clf()
	mpl.close()
